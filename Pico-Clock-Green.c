...
unsigned char alarm_id = 0, alarm_flag = 0, beep_sta = 1, beep_flag = 0,
              beep_on_flag = 0, beep_on_count = 0, scroll_flag = 0,
...
unsigned char Set_time_hour_flag = 0, Set_time_min_flag = 0,
              ...
              change_time_flag = 0;
unsigned char alarm_select_flag = 0, alarm_open_flag = 0, alarm_select_sta = 0,
              alarm_open_sta = 0, hour_temp, min_temp, year_temp, month_temp,
...
unsigned char adc_show_flag = 0, adc_show_time = 6;
TIME_RTC Time_RTC;
...
bool repeating_timer_callback_ms(struct repeating_timer *t) { // 1ms进入一次
  ...
  if (gpio_get(SET_FUNCTION) == 0) //检测设置按钮是否被按下
  {
    ...
    if (KEY_cnt > 50 &&
        KEY_cnt < 300) //少于300ms判断为短按,设置键短按为模式切换
    {
      KEY_cnt = 0;
      if (alarm_id == 1)
        ...
        KEY_Set_flag =
            1; //进入普通设置模式
               //（外部时钟设置、按键声音开关设置、滚动开关设置、时钟显示模式设置）
      beep_start_judge();
      EXIT();
      set_id++;
  ...
  if (gpio_get(UP) == 0) //+标号
  {
    ...
    if (UP_cnt > 50 &&
        UP_cnt < 300) //少于300ms判断为短按,短按+、-键可以改变功能的状态
    {
      UP_cnt = 0;
  ...
  if (gpio_get(DOWN) == 0) //-标号
  {
    ...
    if (Exit_cnt > 0 &&
        Exit_cnt < 300) //少于300ms判断为短按,短按+、-键可以改变功能的状态
    {
      No_operation_count = 0;
      if (set_id == 0) //自动亮度开关
      {
      ...
      Exit_cnt = 0;
               ...
               set_id !=
                   0) //大于300ms判断为长按 ,任何设置模式下,可以退出设置模式
    {
      beep_start_judge();
      Special_Exit();
...
bool repeating_timer_callback_s(struct repeating_timer *t) // 1s 进入一次
{
  ...
  {
    if (alarm_open_sta != 0) {
      if (Time_RTC.dayofweek == alarm_day_select &&
          Ds3231_check_alarm() == true) {
        alarm_star_flag = 1;
        gpio_put(BUZZ, 1);
      }
    }

    if (No_operation_flag == 0 && scroll_start_count == 0 &&
        adc_show_time == 0) {
      update_time = 1;
    }
    Min_count = 0;
  ...
  if (No_operation_flag == 1) {
    ...
    if (No_operation_count == 10) // 10秒内无任何操作则退出设置模式
    {
      ...
      if (alarm_select_sta == 0 && alarm_open_sta != 0 &&
        ...
        Set_alarm1_clock(ALARM_MODE_HOUR_MIN_SEC_MATCHED, 00, alarm_min_temp,
                         alarm_hour_temp, alarm_day_select);
      }
      if (alarm_select_sta != 0 && alarm_open_sta != 0 &&
          alarm_day_select_flag == 1) {
  ...
  for (i = 1; i < 8; i++) {
    if (k > 0) {
      ...
      disp_buf[8 * j + i] =
          ...
          ((ZIKU[dis_char * 7 + i - 1]) << k); //保留需要的数据位
...
static void dis_Timing() {
  ...
  } else if (set_id == 4 && Timing_mode_sta == 1 && Timing_DN_close_flag == 0 &&
             Timing_mode_sta != 2) //倒计时显示
  {
    No_operation_count = 0;
    Timing_DN_flag = 1;
    display_char(0, Timing_min_temp / 10 + '0');
...
static void dis_scroll() {
  ...
  for (unsigned char i = 1; i < 8; i++) {
    ...
    for (unsigned char jr = 0; jr < sizeof(disp_buf) / 8; jr++) {

      if (jr < sizeof(disp_buf) / 8 - 1)
        disp_buf[8 * jr + i] =
            disp_buf[8 * jr + i] >> 1 | disp_buf[8 * jr + i + 8] << 7;
      else
...
static void Alarm_set(uint8_t UP_DOWN_flag) {
  ...
  if (alarm_min_flag == 1) {
    if (UP_DOWN_flag == UP_flag) {
      alarm_min_temp++;
      if (alarm_min_temp == 60)
        alarm_min_temp = 0;
    } else {
      alarm_min_temp--;
      if (alarm_min_temp == 255)
        alarm_min_temp = 59;
    }
  }
...
static void Timing_set(uint8_t UP_DOWN_flag) {
  if (Timing_mode_flag == 1) {
    if (UP_DOWN_flag == UP_flag) {
      Timing_mode_sta++;
      if (Timing_mode_sta == 3)
        Timing_mode_sta = 0;
    } else {
      Timing_mode_sta--;
      if (Timing_mode_sta == 255)
        Timing_mode_sta = 2;
    }
  }
